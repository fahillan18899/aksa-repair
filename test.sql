...
SELECT COUNT(id) FROM stock_opnames;

SELECT history_stock_opnames.total_sparepart AS selisih_jumlah_masuk_keluar_terakhir FROM history_stock_opnames JOIN stock_opnames ON history_stock_opnames.sparepart_id = stock_opnames.id WHERE stock_opnames.id = 34 ORDER BY history_stock_opnames.created_at DESC LIMIT 1;
